...
public class ScaleBothOperator extends Operator {
    ...
    public final Input<RealParameter> otherParameterInput = new Input<>("otherParameter", "this parameter is ALSO scaled in the same way as 'parameter'",
    ...
    public void initAndValidate() {
        ...
        if (indicators != null) {
            ...
            final int otherDataDim = otherParameterInput.get().getDimension();
            final int indsDim = indicators.getDimension();
            if(dataDim != otherDataDim){
                throw new IllegalArgumentException("parameter dimension not compatible with otherParameter dimension");
            }
    ...
    protected boolean outsideBounds(final double value, final double otherValue, final RealParameter param, final RealParameter otherParam) {
        final Double l1 = param.getLower();
        final Double h1 = param.getUpper();

        final Double l2 = otherParam.getLower();
        final Double h2 = otherParam.getUpper();

        return (value < l1 || value > h1) && (otherValue < l2 || otherValue > h2);
    ...
    public double proposal() {
        ...
        try {
            ...
            final RealParameter otherParam = otherParameterInput.get(this);
            ...
            if (scaleAllIndependently) {
                ...
                if (indicators != null) {
                    ...
                    for (int i = 0; i < dim; i++) {
                        if( (impliedOne && (i == 0 || indicator[i-1])) || (!impliedOne && indicator[i]) )  {
                            ...
                            final double otherNewValue = scaleOne * otherParam.getValue(i);

                            hastingsRatio -= Math.log(scaleOne);

                            if (outsideBounds(newValue, otherNewValue, param, otherParam)) {
                                return Double.NEGATIVE_INFINITY;
                            }

                            param.setValue(i, newValue);
                            otherParam.setValue(i, otherNewValue);
                        }
                    }
                }  else {
                ...
                for (int i = 0; i < dim; i++) {
                    ...
                    final double otherNewValue = otherParam.getValue(i) * scale;
                ...
                final double otherOldValue = otherParam.getValue(index);

                if (oldValue == 0 || otherOldValue == 0) {
                    // Error: parameter or otherParameter has value 0 and cannot be scaled
                    return Double.NEGATIVE_INFINITY;
                }

                final double newValue = scale * oldValue;
                final double otherNewValue = scale * otherOldValue;
                ...
                otherParam.setValue(index, otherNewValue);
